...
The repository includes a Render Blueprint for the API and a Vercel SPA rewrite for the frontend.
...
4. Initially set `CLIENT_URL` to the expected Vercel URL.
5. Confirm `/api/health` succeeds after deployment.
...
4. Deploy, then update Render's `CLIENT_URL` with the exact Vercel URL.
5. Redeploy the API after changing `CLIENT_URL`.

The production cookie configuration supports HTTPS cross-site requests. Do not add trailing slashes to `CLIENT_URL` or `VITE_API_URL`.
